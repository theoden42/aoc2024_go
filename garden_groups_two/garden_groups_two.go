...
func main() {
	...
	is_corner := func(i, j int) int {
		count := 0
		if is_boundary(i-1, j, i, j) && is_boundary(i, j+1, i, j) {
			count += 1
		}
		if is_boundary(i, j+1, i, j) && is_boundary(i+1, j, i, j) {
			count += 1
		}
		if is_boundary(i+1, j, i, j) && is_boundary(i, j-1, i, j) {
			count += 1
		...
		if !is_boundary(i-1, j, i, j) && !is_boundary(i, j+1, i, j) && is_boundary(i-1, j+1, i, j) {
			count += 1
		}
		if !is_boundary(i, j+1, i, j) && !is_boundary(i+1, j, i, j) && is_boundary(i+1, j+1, i, j) {
			count += 1
		}
		if !is_boundary(i+1, j, i, j) && !is_boundary(i, j-1, i, j) && is_boundary(i+1, j-1, i, j) {
			count += 1
	...
	dfs = func(i, j int) []int {
		visited[i][j] = true
		count_area := 1
		count_peri := is_corner(i, j)
		fmt.Println(count_peri, i, j)
